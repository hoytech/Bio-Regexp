...
This module is for searching inside DNA or RNA or protein sequences. The sequence to be found is specified by a restricted sub-set of perl regular expressions. The restrictions allow us to manipulate the regexp in various ways described below. As well as regular expression character classes, bases can be expressed in IUPAC short form (which are kind of like character classes themselves).

The goal of this module is to provide a complete search. Given the particulars of a sequence (DNA/RNA/protein, linear molecule/circular plasmid, single/double stranded) it attempts to figure out all of the possible matches, all without any false-positive or duplicated matches.
...
Normally C<T> and C<U> are both compiled into C<[TU]> so your patterns will work on DNA and RNA. If you wish to prevent this, call C<strict_thymine_uracil>.
...
All offsets returned by this module are in "interbase coordinates". Rather than the first base in a sequence being considered "base 1" as most biologists might think of it, or even "base 0" as computer scientists might, in interbase coordinates the first base is the sequence spanning coordinates 0 through 1.
...
Unlike with string indices, the start coordinate can be greater than the end coordinate. This only happens if C<double_stranded> is set (the default for DNA) and the pattern was found on the reverse complement strand. Use C<single_stranded> if you don't want reverse complement matches.
...
Doing a single pass is generally more efficient because of memory locality and has a few other positive side-effects. For instance, we can scan the reverse complement strand using a single pass and therefore avoid copying and reversing the input (which may be quite large).
